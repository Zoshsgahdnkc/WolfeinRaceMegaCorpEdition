...
namespace MCMigrationPatch
{
    ...
    
    {
        ...
        public override void TickInterval(int delta)
        {
            ...
            // 应用Hediff
            var existing = pawn.health?.hediffSet?.GetFirstHediffOfDef(WRMC_DefOfs.WRMC_ClusterTacticsEffect);
            if (hasAlly)
            {
                if (existing == null) pawn.health.AddHediff(WRMC_DefOfs.WRMC_ClusterTacticsEffect);
            }
            // else
            // {
